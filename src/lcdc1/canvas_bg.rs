...
///Field `ALL_BLENDING_BYPASS` reader - if this bit is set, lcdc is in pure dma mode. No blending operation.
pub type AllBlendingBypassR = crate::BitReader;
///Field `ALL_BLENDING_BYPASS` writer - if this bit is set, lcdc is in pure dma mode. No blending operation.
pub type AllBlendingBypassW<'a, REG> = crate::BitWriter<'a, REG>;
///Field `LB_BYPASS` reader - line buffer bypass. Set 1 to bypass line buffer.
pub type LbBypassR = crate::BitReader;
///Field `LB_BYPASS` writer - line buffer bypass. Set 1 to bypass line buffer.
pub type LbBypassW<'a, REG> = crate::BitWriter<'a, REG>;
///Field `H_MIRROR` reader - set 1 to do horizontal mirror for output image
...
impl R {
    ...
    ///Bit 25 - if this bit is set, lcdc is in pure dma mode. No blending operation.
    #[inline(always)]
    pub fn all_blending_bypass(&self) -> AllBlendingBypassR {
        AllBlendingBypassR::new(((self.bits >> 25) & 1) != 0)
    }
    ///Bit 26 - line buffer bypass. Set 1 to bypass line buffer.
    #[inline(always)]
    pub fn lb_bypass(&self) -> LbBypassR {
        LbBypassR::new(((self.bits >> 26) & 1) != 0)
    }
    ///Bit 27 - set 1 to do horizontal mirror for output image
    #[inline(always)]
...
impl W {
    ...
    ///Bit 26 - line buffer bypass. Set 1 to bypass line buffer.
    #[inline(always)]
    pub fn lb_bypass(&mut self) -> LbBypassW<CANVAS_BGrs> {
        LbBypassW::new(self, 26)
    }
    ///Bit 27 - set 1 to do horizontal mirror for output image
    #[inline(always)]
